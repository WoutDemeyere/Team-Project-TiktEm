...
def TikTakBoomStart():
    ...
    for tik in tiks:                # voor elke tik een thread opstarten
        x = threading.Thread(target=TikTakBoomThread, args=(tik.id, timeUltilBoomMAX))
        x.start()
...
def TikTakBoomThread(tikid, timeUltilBoomMAX):
    ...
    while tiktem.game_on == True:
        ...
        if gameResult == "KABAM":
            ...
            print("AFGETIKT")
            # aftik signaal
            tiktem.tiks[tikid].turn_on_delay(0, 255, 255, 1000, 100)
            time.sleep(sleepTime)

        elif gameResult == "STOP":
            tiktem.tiks[tikid].turn_on_delay(255, 0, 255, 700, 200)
